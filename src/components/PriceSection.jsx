...
const PriceSection = ({ onRoomAdd, cartItems, onUpdateQuantity }) => {
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-8">
      ...
      {rooms.map((room) => {
        const cartItem = cartItems.find(item => item.id === room.id);
        return (
          <div
            key={room.id}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mt-4 gap-4">
                ...
                {!cartItem ? (
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      className="w-8 h-8 rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 text-xl flex items-center justify-center"
                      onClick={() => onUpdateQuantity(room.id, Math.max(1, cartItem.quantity - 1))}
                    >
                      -
                    </button>
                    ...
                    <button
                      className="w-8 h-8 rounded-full bg-gray-200 text-gray-700 hover:bg-gray-300 text-xl flex items-center justify-center"
                      onClick={() => onUpdateQuantity(room.id, cartItem.quantity + 1)}
                    >
                      +
                    </button>
                  </div>
